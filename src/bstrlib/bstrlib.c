...
/** \brief Increase the size of the memory backing the bstring b to at least
 *         len.
 */
...
/** \brief Set the size of the memory backing the bstring b to len or
 *         b->slen+1, whichever is larger.  Note that repeated use of this
 *         function can degrade performance.
 */
...
/** \brief Create a bstring which contains the contents of the '\0' terminated
 *         char buffer str.
 */
...
/** \brief Create a bstring which contains the content of the block blk of
 *         length len.
 */
...
/** \brief Create a '\0' terminated char * buffer which is equal to the
 *         contents of the bstring s, except that any contained '\0' characters
 *         are converted to the character in z. This returned value should be
 *         freed with a bcstrfree () call, by the calling application.
 */
...
/** \brief Frees a C-string generated by bstr2cstr ().  This is normally
 *         unnecessary since it just wraps a call to bstr__free (), however,
 *         if bstr__alloc () and bstr__free () have been redefined as a macros
 *         within the bstrlib module (via defining them in memdbg.h after
 *         defining BSTRLIB_MEMORY_DEBUG) with some difference in behaviour
 *         from the std library functions, then this allows a correct way of
 *         freeing the memory that allows higher level code to be independent
 *         from these macro redefinitions.
 */
...
/** \brief Concatenate the bstring b1 to the bstring b0.
 */
...
/** \brief Concatenate the single character c to the bstring b.
 */
...
/** \brief Concatenate a char * string to a bstring.
 */
...
/** \brief Concatenate a fixed length buffer to a bstring.
 */
...
/** \brief Create a copy of the bstring b.
 */
...
/** \brief Overwrite the string a with the contents of string b.
 */
...
/** \brief Overwrite the string a with the middle of contents of string b
...
/** \brief Overwrite the string a with the contents of char * string str.
 *         Note that the bstring a must be a well defined and writable bstring.
 *         If an error occurs BSTR_ERR is returned however a may be partially
 *         overwritten.
 */
...
/** \brief Overwrite the string a with the contents of the block (s, len).
 *         Note that the bstring a must be a well defined and writable bstring.
 *         If an error occurs BSTR_ERR is returned and a is not overwritten.
 */
...
/** \brief Truncate the bstring to at most n characters.
 */
...
/** \brief Convert contents of bstring to upper case.
 */
...
/** \brief Convert contents of bstring to lower case.
 */
...
/** \brief Compare two strings without differentiating between case.  The
 *         return value is the difference of the values of the characters
 *         where the two strings first differ after lower case transformation,
 *         otherwise 0 is returned indicating that the strings are equal. If
 *         the lengths are different, then a difference from 0 is given, but
 *         if the first extra character is '\0', then it is taken to be the
 *         value UCHAR_MAX+1.
 */
...
/** \brief Compare two strings without differentiating between case for at
 *         most n characters.  If the position where the two strings first
 *         differ is before the nth position, the return value is the
 *         difference of the values of the characters, otherwise 0 is returned.
 *         If the lengths are different and less than n characters, then a
 *         difference from 0 is given, but if the first extra character is
 *         '\0', then it is taken to be the value UCHAR_MAX+1.
 */
...
/** \brief Compare two strings for equality without differentiating between
 *         case. If the strings differ other than in case, 0 is returned, if
 *         the strings are the same, 1 is returned, if there is an error, -1 is
 *         returned.  If the length of the strings are different, this function
 *         is O(1).  '\0' termination characters are not treated in any special
 *         way.
 */
...
/** \brief Compare beginning of string b0 with a block of memory of length len
 *         without differentiating between case for equality. If the beginning
 *         of b0 differs from the memory block other than in case (or if b0 is
 *         too short), 0 is returned, if the strings are the same, 1 is
 *         returned, if there is an error, -1 is returned. '\0' characters are
...
/** \brief Compare the string b with the character block blk of length len.
 *         If the content differs, 0 is returned, if the content is the same, 1
 *         is returned, if there is an error, -1 is returned. If the length of
 *         the strings are different, this function is O(1). '\0' characters
...
/** \brief Compare the string b0 and b1.  If the strings differ, 0 is returned,
 *         if the strings are the same, 1 is returned, if there is an error,
 *         -1 is returned.  If the length of the strings are different, this
 *         function is O(1).  '\0' termination characters are not treated in
 *         any special way.
 */
...
/** \brief Compare beginning of string b0 with a block of memory of length len
 *         for equality.  If the beginning of b0 differs from the memory block
 *         (or if b0 is too short), 0 is returned, if the strings are the same,
 *         1 is returned, if there is an error, -1 is returned. '\0' characters
 *         are not treated in any special way.
 */
 ...
 *         contents between the two must be identical with the bstring b with
 *         no '\0' characters for the two contents to be considered equal.
 *         This is equivalent to the condition that their current contents will
 *         be always be equal when comparing them in the same format after
 *         converting one or the other.  If the strings are equal 1 is
 *         returned, if they are unequal 0 is returned and if there is a
...
/** \brief Compare the string b0 and b1.  If there is an error, SHRT_MIN is
 *         returned, otherwise a value less than or greater than zero,
 *         indicating that the string pointed to by b0 is lexicographically
 *         less than or greater than the string pointed to by b1 is returned.
 *         If the the string lengths are unequal but the characters up until
 *         the length of the shorter are equal then a value less than, or
 *         greater than zero, indicating that the string pointed to by b0 is
 *         shorter or longer than the string pointed to by b1 is returned.
 *         0 is returned if and only if the two strings are the same.  If the
 *         length of the strings are different, this function is O(n).  Like
 *         its standard C library counter part strcmp, the comparison does not
 *         proceed past any '\0' termination characters encountered.
 */
...
/** \brief Compare the string b0 and b1 for at most n characters.  If there is
 *         an error, SHRT_MIN is returned, otherwise a value is returned as if
 *         b0 and b1 were first truncated to at most n characters then bstrcmp
 *         was called with these new strings are paremeters.  If the length of
 *         the strings are different, this function is O(n).  Like its standard
 *         C library counter part strcmp, the comparison does not proceed past
 *         any '\0' termination characters encountered.
 */
...
/** \brief Removes characters from pos to pos+len-1 inclusive and shifts the
 *         tail of the bstring starting from pos+len to pos.  len must be
 *         positive for this call to have any effect. The section of the string
 *         described by (pos, len) is clamped to boundaries of the bstring b.
 */
...
/** \brief Free up the bstring.  Note that if b is detectably invalid or not
 *         writable then no action is performed and BSTR_ERR is returned.  Like
 *         a freed memory allocation, dereferences, writes or any other action
 *         on b after it has been bdestroyed is undefined.
 */
 ...
 *         searching forward. If it is found then return with the first
 *         position where it is found, otherwise return BSTR_ERR. Note that
 *         this is just a brute force string searcher that does not attempt
 *         clever things like the Boyer-Moore search algorithm. Because of this
 *         there are many degenerate cases where this can take much longer than
 *         it needs to.
 */
...
/** \brief Search for the bstring b2 in b1 starting from position pos, and
 *         searching backward. If it is found then return with the first
 *         position where it is found, otherwise return BSTR_ERR. Note that
 *         this is just a brute force string searcher that does not attempt
 *         clever things like the Boyer-Moore search algorithm. Because of
 *         this there are many degenerate cases where this can take much longer
 *         than it needs to.
 */
