...
static struct tagbstring module = bsStatic( "hsencode.c" );
...
#if HEATSHRINK_DYNAMIC_ALLOC
heatshrink_encoder* heatshrink_encoder_alloc(
   uint8_t window_sz2, uint8_t lookahead_sz2
) {
   SCAFFOLD_SIZE buf_sz;
   heatshrink_encoder* hse = NULL;
   ...
   scaffold_print_debug( __FILE__, "-- allocated encoder with buffer size of %zu (%u byte input size)\n",
       buf_sz, get_input_buffer_size(hse));
#endif /* HEATSHRINK_DEBUG */
   return hse;
}
...
HSE_sink_res heatshrink_encoder_sink(
   heatshrink_encoder* hse, BYTE* in_buf, SCAFFOLD_SIZE size,
   SCAFFOLD_SIZE* input_size
) {
   ...
   *input_size = cp_sz;
   ...
   scaffold_print_debug( __FILE__, "-- sunk %u bytes (of %zu) into encoder at %d, input buffer now has %u\n",
       cp_sz, size, write_offset, hse->input_size);
   if (cp_sz == rem) {
      scaffold_print_debug( __FILE__, "-- internal buffer is now full\n");
...
HSE_poll_res heatshrink_encoder_poll(
   heatshrink_encoder* hse, BYTE* out_buf, SCAFFOLD_SIZE out_buf_size,
   ...
   if (out_buf_size == 0) {
      scaffold_print_error( &module, "-- MISUSE: output buffer size is 0\n");
   ...
   *output_size = 0;
   ...
   while (1) {
      ...
      /* scaffold_print_debug( __FILE__, "-- polling, state %u (%s), flags 0x%02x\n",
      ...
      default:
#ifdef DEBUG
         scaffold_print_error( &module, "-- bad state %s\n", state_names[hse->state]);
#endif /* DEBUG */
         return HSER_POLL_ERROR_MISUSE;
      }
...
#ifdef DEBUG_HEATSHRINK
   scaffold_print_debug( __FILE__, "-- setting is_finishing flag\n");
#endif /* DEBUG_HEATSHRINK */
...
#ifdef DEBUG_HEATSHRINK
   scaffold_print_debug( __FILE__, "## step_search, scan @ +%d (%d/%d), input size %d\n",
...
#ifdef DEBUG_HEATSHRINK
      scaffold_print_debug( __FILE__, "-- end of search @ %d\n", msi);
#endif /* DEBUG_HEATSHRINK */
...
#ifdef DEBUG_HEATSHRINK
      scaffold_print_debug( __FILE__, "ss Match not found\n");
#endif /* DEBUG_HEATSHRINK */
...
#ifdef DEBUG_HEATSHRINK
      scaffold_print_debug( __FILE__, "ss Found match of %d bytes at %d\n", match_length, match_pos);
#endif /* DEBUG_HEATSHRINK */
...
static HSE_state st_yield_br_index(heatshrink_encoder* hse,
   ...
   if (can_take_byte(oi)) {
#ifdef DEBUG_HEATSHRINK
      scaffold_print_debug( __FILE__, "-- yielding backref index %u\n", hse->match_pos);
#endif /* DEBUG_HEATSHRINK */
...
static HSE_state st_yield_br_length(heatshrink_encoder* hse,
   ...
   if (can_take_byte(oi)) {
#ifdef DEBUG_HEATSHRINK
      scaffold_print_debug( __FILE__, "-- yielding backref length %u\n", hse->match_length);
#endif /* DEBUG_HEATSHRINK */
      if (push_outgoing_bits(hse, oi) > 0) {
...
static HSE_state st_save_backlog(heatshrink_encoder* hse) {
#ifdef DEBUG_HEATSHRINK
   scaffold_print_debug( __FILE__, "-- saving backlog\n");
#endif /* DEBUG_HEATSHRINK */
   save_backlog(hse);
...
#ifdef DEBUG_HEATSHRINK
      scaffold_print_debug( __FILE__, "-- done!\n");
#endif /* DEBUG_HEATSHRINK */
      return HSES_DONE;
   } else if (can_take_byte(oi)) {
#ifdef DEBUG_HEATSHRINK
      scaffold_print_debug( __FILE__, "-- flushing remaining byte (bit_index == 0x%02x)\n", hse->bit_index);
#endif /* DEBUG_HEATSHRINK */
...
static void add_tag_bit(heatshrink_encoder* hse, output_info* oi, uint8_t tag) {
#ifdef DEBUG_HEATSHRINK
   scaffold_print_debug( __FILE__, "-- adding tag bit: %d\n", tag);
#endif /* DEBUG_HEATSHRINK */
...
#ifdef DEBUG_HEATSHRINK
   scaffold_print_debug( __FILE__, "-- scanning for match of buf[%u:%u] between buf[%u:%u] (max %u bytes)\n",
   ...
   for (pos=end - 1; pos - (int16_t)start >= 0; pos--) {
      ...
      if ((pospoint[match_maxlen] == needlepoint[match_maxlen])
         ...
         for (len=1; len<maxlen; len++) {
            if (0) {
#ifdef DEBUG_HEATSHRINK
               scaffold_print_debug( __FILE__, "  --> cmp buf[%d] == 0x%02x against %02x (start %u)\n",
...
#ifdef DEBUG_HEATSHRINK
      scaffold_print_debug( __FILE__, "-- best match: %u bytes at -%u\n",
...
#ifdef DEBUG_HEATSHRINK
   scaffold_print_debug( __FILE__, "-- none found\n");
#endif /* DEBUG_HEATSHRINK */
...
static uint8_t push_outgoing_bits(heatshrink_encoder* hse, output_info* oi) {
   ...
   if (count > 0) {
#ifdef DEBUG_HEATSHRINK
      scaffold_print_debug( __FILE__, "-- pushing %d outgoing bits: 0x%02x\n", count, bits);
#endif /* DEBUG_HEATSHRINK */
...
static void push_bits(heatshrink_encoder* hse, uint8_t count, uint8_t bits,
   ...
   assert(count <= 8);
#ifdef DEBUG_HEATSHRINK
   scaffold_print_debug( __FILE__, "++ push_bits: %d bits, input of 0x%02x\n", count, bits);
#endif /* DEBUG_HEATSHRINK */
   ...
   if (count == 8 && hse->bit_index == 0x80) {
      ...
      for (i=count - 1; i>=0; i--) {
         ...
         if (0) {
#ifdef DEBUG_HEATSHRINK
            scaffold_print_debug( __FILE__, "  -- setting bit %d at bit index 0x%02x, byte => 0x%02x\n",
         ...
         if (hse->bit_index == 0x00) {
            hse->bit_index = 0x80;
#ifdef DEBUG_HEATSHRINK
            scaffold_print_debug( __FILE__, " > pushing byte 0x%02x\n", hse->current_byte);
#endif /* DEBUG_HEATSHRINK */
            oi->buf[(*oi->output_size)++] = hse->current_byte;
            hse->current_byte = 0x00;
         }
...
#ifdef DEBUG_HEATSHRINK
   scaffold_print_debug( __FILE__, "-- yielded literal byte 0x%02x ('%c') from +%d\n",
