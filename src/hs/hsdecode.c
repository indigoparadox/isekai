...
static struct tagbstring module = bsStatic( "hsdecode.c" );
...
heatshrink_decoder* heatshrink_decoder_alloc(
   ...
   scaffold_print_debug( __FILE__, "-- allocated decoder with buffer size of %zu (%zu + %u + %u)\n",
...
#ifdef DEBUG_HEATSHRINK
   scaffold_print_debug( __FILE__, "-- sinking %zd bytes\n", size);
#endif /* DEBUG_HEATSHRINK */
...
HSD_poll_res heatshrink_decoder_poll(
   ...
   *output_size = 0;
   ...
   while (1) {
#ifdef DEBUG_HEATSHRINK
      scaffold_print_debug( __FILE__, "-- poll, state is %d (%s), input_size %d\n",
...
static HSD_state st_yield_literal( heatshrink_decoder* hsd, output_info* oi ) {
   ...
   if (*oi->output_size < oi->buf_size) {
      ...
      c = byte & 0xFF;
#ifdef DEBUG_HEATSHRINK
      scaffold_print_debug( __FILE__, "-- emitting literal byte 0x%02x ('%c')\n", c, isprint(c) ? c : '.');
#endif /* DEBUG_HEATSHRINK */
...
#ifdef DEBUG_HEATSHRINK
   scaffold_print_debug( __FILE__, "-- backref index (msb), got 0x%04x (+1)\n", bits);
#endif /* DEBUG_HEATSHRINK */
   if (bits == NO_BITS) {
      return HSDS_BACKREF_INDEX_MSB;
   }
...
#ifdef DEBUG_HEATSHRINK
   scaffold_print_debug( __FILE__, "-- backref index (lsb), got 0x%04x (+1)\n", bits);
#endif /* DEBUG_HEATSHRINK */
   if (bits == NO_BITS) {
      return HSDS_BACKREF_INDEX_LSB;
   }
...
#ifdef DEBUG_HEATSHRINK
   scaffold_print_debug( __FILE__, "-- backref count (msb), got 0x%04x (+1)\n", bits);
#endif /* DEBUG_HEATSHRINK */
   if (bits == NO_BITS) {
...
#ifdef DEBUG_HEATSHRINK
   scaffold_print_debug( __FILE__, "-- backref count (lsb), got 0x%04x (+1)\n", bits);
#endif /* DEBUG_HEATSHRINK */
   if (bits == NO_BITS) {
      return HSDS_BACKREF_COUNT_LSB;
   }
...
static HSD_state st_yield_backref( heatshrink_decoder* hsd, output_info* oi) {
   ...
   if (count > 0) {
...
#ifdef DEBUG_HEATSHRINK
      scaffold_print_debug( __FILE__, "-- emitting %zu bytes from -%u bytes back\n", count, neg_offset);
#endif /* DEBUG_HEATSHRINK */
      ...
      for (i=0; i<count; i++) {
         ...
         hsd->head_index++;
#ifdef DEBUG_HEATSHRINK
         scaffold_print_debug( __FILE__, "  -- ++ 0x%02x\n", c);
#endif /* DEBUG_HEATSHRINK */
      }
...
static uint16_t get_bits(heatshrink_decoder* hsd, uint8_t count) {
...
#ifdef DEBUG_HEATSHRINK
   scaffold_print_debug( __FILE__, "-- popping %u bit(s)\n", count);
#endif /* DEBUG_HEATSHRINK */
   ...
   for (i = 0; i < count; i++) {
      if (hsd->bit_index == 0x00) {
         if (hsd->input_size == 0) {
#ifdef DEBUG_HEATSHRINK
            scaffold_print_debug( __FILE__, "  -- out of bits, suspending w/ accumulator of %u (0x%02x)\n",
...
#ifdef DEBUG_HEATSHRINK
         scaffold_print_debug( __FILE__, "  -- pulled byte 0x%02x\n", hsd->current_byte);
#endif /* DEBUG_HEATSHRINK */
      ...
      if (hsd->current_byte & hsd->bit_index) {
         ...
         if (0) {
#ifdef DEBUG_HEATSHRINK
            scaffold_print_debug( __FILE__, "  -- got 1, accumulator 0x%04x, bit_index 0x%02x\n",
         ...
         if (0) {
#ifdef DEBUG_HEATSHRINK
            scaffold_print_debug( __FILE__, "  -- got 0, accumulator 0x%04x, bit_index 0x%02x\n",
                accumulator, hsd->bit_index);
#endif /* DEBUG_HEATSHRINK */
         }
      }
   ...
   if (count > 1) {
#ifdef DEBUG_HEATSHRINK
      scaffold_print_debug( __FILE__, "  -- accumulated %08x\n", accumulator);
#endif /* DEBUG_HEATSHRINK */
   }
   return accumulator;
}
...
static void push_byte(heatshrink_decoder* hsd, output_info* oi, uint8_t byte) {
#ifdef DEBUG_HEATSHRINK
   scaffold_print_debug( __FILE__, " -- pushing byte: 0x%02x ('%c')\n", byte, isprint(byte) ? byte : '.');
#endif /* DEBUG_HEATSHRINK */
